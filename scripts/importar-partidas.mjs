 ...
 * el que lleva las correcciones. Nunca se borra nada de lo guardado.
 */
...
export function leerGuardado(ruta) {
  if (!existsSync(ruta)) return { actualizado: null, partidas: [], maestria: {}, envios: [] };
  const g = JSON.parse(readFileSync(ruta, 'utf8'));
  return { actualizado: g.actualizado ?? null, partidas: Array.isArray(g.partidas) ? g.partidas : [], maestria: g.maestria ?? {}, envios: Array.isArray(g.envios) ? g.envios : [] };
}
...
export async function importar(texto, ruta, { incidencia = null, ahora = new Date() } = {}) {
  ...
  const fundido = fundirPerfil(
    { mastery: perfil.mastery, partidas: perfil.partidas },
    { mastery: guardado.maestria, partidas: guardado.partidas },
  );
  ...
  return { antes: guardado.partidas.length, enElEnvio: perfil.partidas.length, ahora: fundido.partidas.length, nuevas, heroes: Object.keys(fundido.mastery).length };
}
...
if (process.argv[1] && process.argv[1].endsWith('importar-partidas.mjs')) {
  ...
  console.log(`Partidas guardadas antes: ${r.antes} · en el envío: ${r.enElEnvio} · ahora: ${r.ahora} (${r.nuevas} nuevas) · héroes con maestría: ${r.heroes}`);
}
