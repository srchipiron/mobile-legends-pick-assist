#!/usr/bin/env node
/**
 * Comprobaciones sobre el CSS que no requieren navegador: nada esencial
 * oculto en móvil, variables declaradas, consultas de medios AL FINAL y toda
 * clase que use la interfaz con estilo. Recorre TODOS los .jsx de src/, no
 * una lista fija: un componente nuevo queda vigilado sin apuntarlo.
 *
 *   node scripts/comprobar/css.mjs [ruta/al/styles.css]
 */
...
function jsxDe(ruta) {
  if (statSync(ruta).isDirectory()) return readdirSync(ruta).flatMap((n) => jsxDe(join(ruta, n)));
  return ruta.endsWith('.jsx') ? [ruta] : [];
}
// El directorio de la interfaz se puede pasar como segundo argumento, para
// que la prueba le dé un JSX con una clase inventada y vea que se entera.
const jsx = jsxDe(process.argv[3] ? resolve(process.argv[3]) : resolve(ROOT, 'src'))
