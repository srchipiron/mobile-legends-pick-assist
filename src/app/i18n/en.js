...
export default {
  ...
  'baneos.segun': 'by ban rate in {rango}',
  ...
  'pie.rangoFuerza': 'Rank: {rango} · win rates from {usado} (the «Meta» sheet says why) · {dias}-day window',
  ...
  'app.tuPick': 'You',
  'app.tuPickDe': 'Your pick: {nombre}',
  'app.soltarPick': 'Release {nombre}: no longer your pick',
  'pick.loCojo': 'Taking it: {nombre} is my pick',
  'pick.soltar': 'Release {nombre}',
  'pick.tuyo': 'your pick',
  ...
  'registro.otroHeroe': 'Another hero (you were put in another lane)…',
  ...
  'hist.boton': 'Matches',
  'meta.boton': 'Meta',
  'meta.tierPista': 'The letter is the mlbb.gg tier: curated opinion, not data. Shown to compare, and it does not count towards the score: measured, it adds nothing to what the win rate already says.',
  'pick.tier': '{tier} on mlbb.gg',
  'pick.wrLinea': '{linea} {pct}%',
  'pick.wrLineaTitulo': 'Its win rate playing {linea} (a few weeks of Glory). The one above mixes all its lanes. Shown, not scored.',
  ...
  'meta.wrLinea': ' · as {linea} {pct}%',
  'meta.wrLineaPista': '“As Roam 42.8%” is that hero\'s win rate playing THAT lane (a few weeks of Glory): the big one mixes every lane it plays, and some heroes do very differently depending on where (Saber: jungle 53.8%, roam 42.4%). Shown, not scored: measured against matches, it does not improve the prediction.',
  ...
  'hist.enviarPistaAuto': 'Automatic upload is on: this button uploads right now, without opening anything.',
  'hist.subida': 'Uploaded: {n} {n|match|matches} with the app, in issue #{numero}. The bot replies there with the measurement.',
  'hist.auto': 'Automatic upload',
  'hist.autoPista': 'With a GitHub token, every match you log is uploaded to the project on its own (same issue, no browser, no login). The token stays on this phone and only travels to api.github.com.',
  'hist.autoPlaceholder': 'GitHub token (github_pat_…)',
  'hist.autoActivar': 'Turn on automatic upload',
  'hist.autoQuitar': 'Turn off automatic upload',
  'hist.autoCrear': 'Create the token on GitHub',
  'hist.autoComo': '(Fine-grained token → Repository access: only this repository → Permissions → Issues: Read and write). It can only open and edit issues in this repository: nothing to do with your game account.',
  'hist.autoSubiendo': 'Uploading…',
  'hist.autoPendiente': 'There are changes to upload: they go up on their own in a few seconds.',
  'hist.autoUltima': 'Last upload: {fecha}, in issue #{numero}. All up to date.',
  'hist.autoNada': 'On. It will upload as soon as you log a match.',
  'hist.autoCuerpo': 'Uploaded automatically from the app. The code below carries my logged matches and my mastery; the bot merges it into historial/partidas.json and replies with the measurement.',
  'hist.error.token': 'GitHub does not accept the token (expired or copied wrong). Turn the upload off and on again with a new one.',
  'hist.error.permiso': 'The token cannot open issues in this repository: when creating it, Repository access → only this repository, and Permissions → Issues: Read and write.',
  'hist.error.red': 'No connection: it will upload on its own when the network is back.',
  'hist.error.otro': 'GitHub replied with an error. It will retry later.',
  'hist.error.ocupado': 'An upload is already running.',
  'hist.error.sinToken': 'Automatic upload is not on.',
  ...
  'termino.equilibrioLargo': 'Damage: physical/magic mix on your side, minus theirs (measured: no mix, you lose)',
  ...
  'regla.equilibraDano': 'brings the {tipo} damage the team was missing',
  ...
  'peligro.fuerte': 'wins {pct}% in {rango}',
  ...
  'analisis.tuPickPorDebajo': 'With {yo} you are {puntos} points below {mejor}, the current #1.',
