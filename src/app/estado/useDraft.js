...
export function useDraft() {
  ...
  useEffect(() => {
    guardar(CLAVES.draft, { enemies: draft.enemigos, allies: draft.aliados, bans: draft.baneos, enemyRoam: draft.rivalMarcado, fase: draft.fase });
  }, [draft]);

  const anadir = useCallback((bando, heroe) => setDraft((d) => {
    const lista = d[bando];
    if (lista.length >= TOPES[bando] || lista.includes(heroe.name)) return d;
    return { ...d, [bando]: [...lista, heroe.name] };
  }), []);
  ...
  const reiniciar = useCallback(() => setDraft({ enemigos: [], aliados: [], baneos: [], rivalMarcado: null, fase: 'baneos' }), []);
  ...
  const limpiarDesconocidos = useCallback((conocidos) => setDraft((d) => {
    ...
    if (enemigos === d.enemigos && aliados === d.aliados && baneos === d.baneos && rivalMarcado === d.rivalMarcado) return d;
    return { ...d, enemigos, aliados, baneos, rivalMarcado };
  }), []);

  return { ...draft, anadir, quitar, alternarBaneo, marcarRival, setFase, reiniciar, limpiarDesconocidos };
}
